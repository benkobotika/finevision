...
)  # Import custom modules (here, mmpretrain.models for Swin backbone)
...
model = dict(
    ...
    # Backbone: Swin-Base from mmpretrain
    backbone=dict(
        type="mmpretrain.SwinTransformer",  # Use Swin from mmpretrain
        arch="base",  # Swin-Base architecture
        drop_path_rate=0.3,  # Stochastic depth
        out_indices=(0, 1, 2, 3),  # Output all 4 stages for FPN
        init_cfg=dict(
            type="Pretrained",
    ...
    # The FPN neck adapts multi-scale Swin features for detection heads
    neck=dict(
        type="FPN",
        in_channels=[128, 256, 512, 1024],  # Swin-Base output channels
        out_channels=256,
        num_outs=5,
    ),
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            "absolute_pos_embed": dict(decay_mult=0.0),  # Swin-specific
            "relative_position_bias_table": dict(decay_mult=0.0),  # Swin-specific
            "norm": dict(decay_mult=0.0),  # All normalization layers
