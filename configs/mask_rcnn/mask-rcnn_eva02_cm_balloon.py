...
data_root = (
    "../datasets/ballondatasets/balloon/"  # Root directory for the balloon dataset
)
...
train_pipeline = [  # Training data processing pipeline
    ...
    ),  # Resize image to 224x224, do not keep aspect ratio
    dict(
    ...
    dict(
        type="PhotoMetricDistortion",  # Apply photometric distortions
        brightness_delta=32,
...
test_pipeline = [  # Test data processing pipeline
    ...
    dict(
        type="Resize", scale=(224, 224), keep_ratio=False
    ),  # Resize image to 224x224, keep aspect ratio
    dict(
        type="LoadAnnotations", with_bbox=True, with_mask=True
    ),  # Load bounding box and mask annotations
    dict(
...
val_evaluator = dict(
    ...
    ann_file=data_root
    + "val/annotation_coco.json",  # Annotation file for validation set
...
default_hooks = dict(
    ...
    ),  # Log training info every 10 iterations
