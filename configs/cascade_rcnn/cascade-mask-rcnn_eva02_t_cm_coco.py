# ============================================================
# Cascade Mask R-CNN + ViT-EVA02-Tiny
...
val_evaluator = dict(
    ...
    ann_file=data_root + 'annotations/instances_val2017.json',  # Annotation file for validation set
...
# ------------------------------------------------------------
model = dict(
    type='CascadeRCNN',  # Use Cascade R-CNN architecture
    ...
    neck=dict(
        ...
        out_channels=256,  # Output channels for detection heads
    ...
    roi_head=dict(
        type='CascadeRoIHead',
        num_stages=3,  # Three refinement stages
        stage_loss_weights=[1, 0.5, 0.25],  # Loss weight for each stage
    ...
    train_cfg=dict(
        ...
        rcnn=[
            ...
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
                    pos_iou_thr=0.6,  # IoU threshold for positive RoIs
                    neg_iou_thr=0.6,  # IoU threshold for negative RoIs
                    min_pos_iou=0.6,
                    match_low_quality=False,
                    ignore_iof_thr=-1),
                sampler=dict(
            ...
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
                    pos_iou_thr=0.7,  # IoU threshold for positive RoIs
                    neg_iou_thr=0.7,  # IoU threshold for negative RoIs
                    min_pos_iou=0.7,
                    match_low_quality=False,
                    ignore_iof_thr=-1),
                sampler=dict(
